      ...
      for(let i=0; i<bills.length; i++){
        ...
        if (this.style.opacity!=0){
         ...
         /* bills[0].style.top = event.clientY-62+"px";
          bills[0].style.left = event.clientX-150+"px";
          function moveAt(e){
            bills[0].style.top = e.clientY-62+"px";
            bills[0].style.left = e.clientX-150+"px";    
          }
          document.addEventListener("mousemove", moveAt)
          document.onmouseup = function(){
            document.removeEventListener("mousemove", moveAt);
          }
          bills[0].ondragstart = function(){return false;}  
        }
        
        //bills[1].onmousedown = function(event){
          bills[1].style.top = event.clientY-62+"px";
          bills[1].style.left = event.clientX-150+"px";
          function moveAt(e){
            bills[1].style.top = e.clientY-62+"px";
            bills[1].style.left = e.clientX-150+"px";    
          }
          document.addEventListener("mousemove", moveAt)
          document.onmouseup = function(){
            document.removeEventListener("mousemove", moveAt);
          }
          bills[1].ondragstart = function(){return false;}        
        }
        
        //bills[2].onmousedown = function(event){
          bills[2].style.top = event.clientY-62+"px";
          bills[2].style.left = event.clientX-150+"px";
          function moveAt(e){
            bills[2].style.top = e.clientY-62+"px";
            bills[2].style.left = e.clientX-150+"px";    
          }
          document.addEventListener("mousemove", moveAt)
          document.onmouseup = function(){
            document.removeEventListener("mousemove", moveAt);
          }
          bills[2].ondragstart = function(){return false;}        
        }        
      }*/
      ...
      function getCoffee(coffeeName,cost){
        if (money.value >= cost){
          ...
          displayInfo.innerText = "Недостаточно средств!";
      }
